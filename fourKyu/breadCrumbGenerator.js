// https://www.codewars.com/kata/563fbac924106b8bf7000046/javascript
...
// generateBC("mysite.com/pictures/holidays.html", " : ") == '<a href="/">HOME</a> : <a href="/pictures/">PICTURES</a> : <span class="active">HOLIDAYS</span>'
                                                           // <a href="/">HOME</a> : <a href="/pictures/">PICTURES</a> : <span class="active">HOLIDAYS.HTML</span>
const url = "https://www.linkedin.com/in/giacomosorbi"
console.log(url.split("/")[1].toUpperCase())
...
function generateBC(url, separator) {
    ...
    if(filler.length > 1){
        for(var i = 0; i < filler.length; i++){
            if(filler[i].length >2){
                acro += filler[i][0].toUpperCase()
            }
        }
        filler = acro
    }else{ 
        filler = filler[0].toUpperCase()
    }
    section2 = ""
    filler2 = arr[2].split("-")
    lastVar = ""
    if(filler2.length > 1){
        for(var i = 0; i < filler2.length; i++){
            if(i == filler2.length-1){
                lastVar += filler2[i].toUpperCase() 
            }else{lastVar += filler2[i].toUpperCase()+ " "}
        }
    } else{
        lastVar = arr[2].split(".")[0].toUpperCase()
    }
    if(arr.length > 3){
        output+=`<a href="/`+arr[1]+`/">`+filler+"</a>"+separator
        output+='<a href="'
        for(var i = 1; i< arr.length-2; i++){
            output+="/"+arr[i]
        }
        output+='/">'+arr[2].toUpperCase()+'</a>'+separator+'<span class="active">'+arr[arr.length-2].split("?")[0].toUpperCase()+"</span>"
    }else{
        output += `<a href="/`+arr[1]+`/">`+filler+"</a>"+separator+'<span class="active">'+lastVar.split("?")[0]+"</span>"
    }
...
// - instead got: &lt;a href="/"&gt;HOME&lt;/a&gt; / &lt;a href="/users/"&gt;USERS&lt;/a&gt; / &lt;span class="active"&gt;GIACOMOSORBI?REF=CODEWARS&lt;/span&gt;